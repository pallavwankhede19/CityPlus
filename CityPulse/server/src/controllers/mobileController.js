  ...
  try {
    ...
    const evaluatedRoutes = await Promise.all(routes.map(async (r, index) => {
      ...
      return {
        ...
        steps: r.steps || [], // 🚀 Capture real turn instructions
    ...
    sortableRoutes.sort((a, b) => (
      ...
      (b.signals_count - a.signals_count) // Priority-tie: Choose route with MORE data/signals
    ));
    ...
    return res.status(200).json({
      best_route: {
        ...
        steps: bestRoute.steps || [], // 🚀 Real-time turn instructions added
...
async function scoutSignalsAlongPolyline(req, res, next) {
  try {
    const coords = req.body?.polyline; // List of {lat, lng}
    if (!Array.isArray(coords) || coords.length < 2) {
      return res.status(200).json({ signals: [] });
    }

    // Ensure we pass the objects directly as required by coordsToWKT
    const routeWkt = polyline.coordsToWKT(coords);
    ...
    try {
      ...
      logger.error(`Scout signals DB failure: ${error.message}`);
      signals = [];
    }

    const signalsWithDelay = (signals || []).map(normalizeSignal);
    return res.status(200).json({ signals: signalsWithDelay });
  } catch (error) {
    logger.error(`Error in scoutSignalsAlongPolyline: ${error.message}`);
    return next(error);
  }
}

module.exports = { getRouteSignals, scoutSignalsAlongPolyline };
