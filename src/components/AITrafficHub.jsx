...
export function AITrafficHub() {
  const setTrafficSignal = useStore((s) => s.setTrafficSignal);
  ...
  const applyOptimization = async () => {
    try {
      ...
      if (res.ok) setTrafficSignal('green');
    } catch (e) {
  ...
  const manualOverride = async () => {
    try {
      const res = await fetch(`${API_URL()}/signal`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ signal: 'red' }),
      });
      if (res.ok) setTrafficSignal('red');
    } catch (e) {
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="lg:w-1/3 space-y-6">
          <div className="bg-surface-container-lowest p-8 rounded-[2rem] shadow-xl border border-surface-container-highest">
            ...
            <button onClick={applyOptimization} className="w-full bg-primary hover:bg-primary-container text-white font-extrabold py-5 rounded-2xl shadow-lg shadow-primary/20 transition-all flex items-center justify-center gap-2 mb-4 group cursor-pointer">
              <Zap size={20} className="text-secondary-fixed-dim group-hover:rotate-90 transition-transform" />
              Apply AI Optimization
            </button>
            <div className="grid grid-cols-2 gap-3">
              <button onClick={manualOverride} className="bg-surface-container-high hover:bg-surface-container-highest text-on-surface text-[10px] font-black py-3 rounded-xl uppercase transition-colors cursor-pointer">
                Manual Override
              </button>
