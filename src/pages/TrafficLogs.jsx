...
export function TrafficLogs() {
  ...
  const filteredLogs = query
    ? trafficLogs.filter(l => l.title?.toLowerCase().includes(query.toLowerCase()) || l.desc?.toLowerCase().includes(query.toLowerCase()))
    : trafficLogs;

  const exportCSV = () => {
    const rows = [['ID','Title','Time','Description','Tags'],
      ...trafficLogs.map(l => [l.id, l.title, l.time, l.desc, (l.tags || []).join('|')])
    ];
      ...
      <div className="bg-surface-container-lowest rounded-xl shadow-sm overflow-hidden">
        <div className="p-8 border-b border-outline-variant/10 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            <button onClick={exportCSV} className="px-4 py-2 bg-surface-container-low text-primary text-xs font-bold rounded-lg hover:bg-primary-fixed transition-colors">Export CSV</button>
            <button onClick={() => logTopRef.current?.scrollIntoView({ behavior: 'smooth' })} className="px-4 py-2 bg-primary text-white text-xs font-bold rounded-lg shadow-lg shadow-primary/20">Live View</button>
          </div>
        </div>
        
        <div className="p-8">
          <div className="relative space-y-12">
            <div ref={logTopRef}></div>
            {/* Connection Line */}
